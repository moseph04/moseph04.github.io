(function(window, createjs, opspark, _) {
  ...
  const
    //Question: are all of these constants objects that have already been made? Where are they? 
  ...
  engine //Question: Why no variable declaration for engine? 
  ...
  function update(event) {
    const //Question: are constants global or function scoped?
  ...
  function createPaddle({ width = 20, height = 100, x = 0, y = 0, color = '#CCC' } = {}) {
    const paddle = draw.rect(width, height, color); // Question: what does rect() do? 
    paddle.x = x;
    paddle.y = y;
    return paddle;
  }
